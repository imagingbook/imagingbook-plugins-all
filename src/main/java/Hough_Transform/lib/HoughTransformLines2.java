 ...
 * @version 2018/12/04
 */
public class HoughTransformLines2 {
	...
	private final List<Point>[][] accumulatorBacklinks;
	 ...
	 */
	public HoughTransformLines2(ImageProcessor I, Parameters params) {
	 ...
	 * @param params parameter object.
	 */
	public HoughTransformLines2(Point2D[] points, int width, int height, Parameters params) {
	...
	// members variables).
	private HoughTransformLines2(int width, int height, Parameters params) {
		...
		this.accumulatorBacklinks = makeAccumulatorBacklinks();
	}
	
	private List<Point>[][] makeAccumulatorBacklinks() {
		@SuppressWarnings("unchecked")
		List<Point>[][] abl = new ArrayList[accWidth][accHeight];
		for (int i = 0; i < accWidth; i++) {
			for (int j = 0; j < accHeight; j++) {
				abl[i][j] = new ArrayList<>();
			}
		}
		return abl;
	}
...
//			double r = x * Math.cos(theta) + y * Math.sin(theta);
			...
			if (r0 >= 0 && r1 < accHeight) {
				double alpha = ri - r0;			// balanced insertion
				acc[ai][r0] += (1.0 - alpha);
				acc[ai][r1] += alpha;
				
				// for testing only
				Point p = new Point((int)u, (int)v);	// register contributing image points
				accumulatorBacklinks[ai][r0].add(p);
				accumulatorBacklinks[ai][r1].add(p);
			}
		}
	}
